  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="staffList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.selfState == '1' && scope.row.reviewState == '0'"
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['assess:staff:edit']"
          >审批</el-button>
          <el-button
      ...
      <h3>考核项</h3>
        <template v-for="(item, index) in form.assessItems">
          <el-descriptions title="" :column="3" border>
            ...
            <el-descriptions-item label="自评分" :span="3">{{item.selfScore}}</el-descriptions-item>
            <el-descriptions-item label="审批分" :span="2">
              <el-input-number v-model="item.reviewScore" :precision="1" controls-position="right" :step="0.5" :min="0" :max="item.score" @change="changeSelfVal($event)">
              </el-input-number>
            </el-descriptions-item>
           <el-descriptions-item label="审批加权分" >
              <el-input readonly :value="item.reviewScore*((10-item.ratio)/10)" :id="'input-weight-'+String(item.id)"></el-input>
            </el-descriptions-item>
          </el-descriptions>
        </template>
